  ...
  document.getElementById('detail-content').innerHTML=`
    ...
    <div class="info-card">
      ...
      <div class="dish-meta">
        ...
        <span class="dish-chip">${DIFF_MAP[r.difficulty]}</span>
        ${r.prepTime?`<span class="dish-chip">⏱ ${fmtDuration(r.prepTime)}</span>`:''}
        <span class="dish-chip">${r.servings}人份</span>
      </div>
      ${r.tags.length?`<div style="margin-top:8px">${r.tags.map(t=>`<span style="font-size:11px;color:var(--c-blue);margin-right:8px">#${t}</span>`).join('')}</div>`:''}
      ${r.sourceUrl?`<div style="font-size:11px;color:var(--c-t3);margin-top:8px">来源：${r.sourceName||r.sourceUrl}</div>`:''}
    </div>
    <div class="ing-section"><div style="font-weight:600;margin-bottom:10px;font-size:15px">🥬 食材清单（${r.servings}人份）</div>
      ${r.ingredients.length?r.ingredients.map(i=>`<div class="ing-row"><span>${i.canonicalName}</span>${i.amount?`<span style="color:var(--c-t2)">${i.amount}</span>`:''}</div>`).join(''):'<div style="font-size:13px;color:var(--c-t3);padding:12px 0;text-align:center">暂无食材信息</div>'}
    </div>
    ${r.steps.length?`<div class="step-section"><div style="font-weight:600;margin-bottom:12px;font-size:15px">📝 烹饪步骤</div>
      ${r.steps.map(s=>`<div class="step-item"><div class="step-num">${s.order}</div><div class="step-desc">${esc(s.description)}${s.imagePath?`<br><img src="${s.imagePath}" style="max-width:100%;border-radius:8px;margin-top:8px">`:''}</div></div>`).join('')}
    </div>`:''}
...
function editRecipe(id){
  ...
  if(editingRecipeId){
    // Fix #2: load existing data when editing
    ...
    editStepRows=r?.steps?.length?r.steps.map(s=>({id:uid(),desc:s.description||'',img:s.imagePath||''})):[{id:uid(),desc:'',img:''}];
  }else{
    // New: start blank
    editCoverDataUrl='';
    editIngredientRows=[{id:uid(),val:''}];
    editStepRows=[{id:uid(),desc:'',img:''}];
  }
  page('edit');
}
function renderEditForm(){
  ...
  // 优先用 DOM 中的输入值（新增菜谱时用户已输入的），否则用已有数据
  ...
  document.getElementById('edit-form').innerHTML=`
    ...
    <div class="form-group"><label class="form-label">份量</label><input class="form-input" id="ef-serv" type="number" value="${esc(serv)}" min="1" max="20"></div>
    <div class="form-group"><label class="form-label">食材 <span class="add-row" onclick="addIngRow()">+ 添加</span></label>
      ${editIngredientRows.map((ir,i)=>`<div class="ing-row-edit"><span style="font-size:11px;color:var(--c-t3);width:20px">${i+1}</span>
        <input class="form-input ing-input" value="${esc(ir.val)}" placeholder="如：鸡蛋 2个（回车新增）"
          onkeydown="saveIngVal(${i},this);if(event.key==='Enter'){event.preventDefault();addIngRow();setTimeout(()=>{const el=document.querySelectorAll('.ing-input')[${i+1}];if(el)el.focus()},50)}">
        <span class="row-del" onclick="editIngredientRows.splice(${i},1);renderEditForm()">✕</span></div>`).join('')}
    </div>
    <div class="form-group"><label class="form-label">步骤 <span class="add-row" onclick="addStepRow()">+ 添加</span></span></label>
      ${editStepRows.map((sr,i)=>`<div class="step-row-edit">
        <span style="font-weight:600;color:var(--c-blue);font-size:16px;width:22px">${i+1}</span>
        <textarea class="form-textarea step-textarea" placeholder="第${i+1}步…（回车新增）"
          onkeydown="saveStepVal(${i},this);if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();addStepRow();setTimeout(()=>{const el=document.querySelectorAll('.step-textarea')[${i+1}];if(el)el.focus()},50)}" style="min-height:56px">${esc(sr.desc)}</textarea>
        <span class="row-del" onclick="editStepRows.splice(${i},1);renderEditForm()">✕</span></div>`).join('')}
    </div>
    ...
    <div class="form-group"><label class="form-label">来源链接</label><input class="form-input" id="ef-source" value="${esc(source)}" placeholder="选填"></div>
...
function saveRecipe(){
  ...
  document.querySelectorAll('.step-textarea').forEach((el,i)=>{if(editStepRows[i])editStepRows[i].desc=el.value});
  ...
  const steps=editStepRows.filter(s=>s.desc.trim()).map((s,i)=>({order:i+1,description:s.desc.trim(),imagePath:s.img||null,duration:null}));
  const input={
    name,category:editingRecipeId?(getRecipeById(editingRecipeId)?.category||'meat'):'meat',
    subcategory:document.getElementById('ef-subcat').value.trim()||'',
    coverImagePath:editCoverDataUrl||null,ingredients,steps,
    prepTime:editingRecipeId?(getRecipeById(editingRecipeId)?.prepTime||null):null,
    servings:Number(document.getElementById('ef-serv').value)||2,
    difficulty:editingRecipeId?(getRecipeById(editingRecipeId)?.difficulty||2):2,
    sourceUrl:document.getElementById('ef-source').value.trim()||null,sourceName:null,
      ...
      for(var j=0;j<mergedList.length;j++){
        ...
        h+='<div class="shop-item '+(it.checked?'done':'')+'">'+
          ...
          '<div class="shop-info" onclick="showShopSource(\''+it.name+'\')">'+
            ...
            '<div style="font-size:11px;color:var(--c-t3);margin-top:2px">'+amt+' <span style="margin-left:6px">'+it.recipes.map(function(rr){return rr.rname}).join('、')+'</span></div>'+
          '</div>'+
        '</div>';
      }
    }else{
...
function deleteCheckedIngredients(){var list=generateShopList(getCheckedRecipeIds());var checked=list.filter(function(i){return i.checked});if(!checked.length){toast('请先勾选要删除的食材');return}if(confirm('确定从清单中删除 '+checked.length+' 种食材吗？需要重新添加菜谱才能恢复。')){checked.forEach(function(i){addShopExcluded(i.name)});if(!generateShopList(getCheckedRecipeIds()).length)clearShopList();toast('已删除勾选的食材');renderShopList()}}
...
function parseSingleRecipe(text){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    // 取第一个有意义的短行（2-25字，不含分隔符）
    if(l.length>=2&&l.length<=25&&!/[，,、]/.test(l)){
      titleIdx=i;break;
    }
  }
  const name=lines[titleIdx]||'未知菜谱';
